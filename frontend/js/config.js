(function () {
  ...
  function select(i){ idx=(i+PRESETS.length)%PRESETS.length; localStorage.setItem(KEY,String(idx)); apply(PRESETS[idx]); }
  function autoTick(){
    if(!autoplay) return;
    const p = PRESETS[idx];
    p.a = (p.a + .6) % 360; p.b = (p.b + .9) % 360;
    apply(p); requestAnimationFrame(autoTick);
  }
  function buildPalette(){
    const t = toggle(); if(!t || t.querySelector('.palette')) return;
    const pop = document.createElement('div'); pop.className='palette';
    PRESETS.forEach((p,i)=>{ const s=document.createElement('button'); s.className='swatch';
      s.style.background=`linear-gradient(135deg,hsl(${p.a},100%,70%),hsl(${p.b},100%,70%))`;
      s.title=p.name; s.onclick=(e)=>{e.stopPropagation(); autoplay=false; localStorage.setItem(AUTO,'0'); select(i); pop.remove();};
      pop.appendChild(s);
    });
    t.appendChild(pop);
  ...
  function init(){
    ...
    t.addEventListener('contextmenu', (e)=>{ e.preventDefault(); autoplay=!autoplay; localStorage.setItem(AUTO, autoplay?'1':'0'); if(autoplay) requestAnimationFrame(autoTick); });
