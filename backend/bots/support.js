// Support bot: policy KB first, escalate cleanly, then general LLM fallback.
...
const ORDER = /\b(#?\d{6,})\b/;
const INAPP = /(sex|sexual|nude|nsfw|hate|racist|suicide|self\-harm)/i;
const HUMAN = /(human|agent|person|someone|real support|call me|ring me|speak to)/i;
const YES = /^(y|yes|yeah|yep|please|ok|okay|sure)\b/i;

export default async function supportBot(message){
  const text = (message||'').trim(); const lc = text.toLowerCase();
  if(!text) return "Hello — ask me about returns, shipping, warranty or support hours.";
  if(INAPP.test(lc)) return "I’m not able to help with that. If you have a question about orders, shipping, or returns, I’m happy to assist.";

  if (HUMAN.test(lc) || YES.test(lc)){
    const email = text.match(EMAIL)?.[0];
    const order = text.match(ORDER)?.[1];
    const bits = []; if(order) bits.push(`order ${order}`); if(email) bits.push(`e-mail ${email}`);
  ...
  for(const item of KB){ if(item.q.some(k=> lc.includes(k))) return item.a; }

  if (USE_LLM) {
    const ans = await generalAnswer(text, "You are a retail/customer support assistant. If unsure, say you don’t know and offer human handoff.");
    return ans || "I couldn’t find that in our documents. Would you like me to connect you with a human agent?";
  }
